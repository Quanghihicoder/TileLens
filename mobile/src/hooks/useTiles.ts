...
export const useTiles = (
  ...
  const calculateVisibleArea = useCallback(() => {
    if (containerDimensionsRef.current.width == 0 && containerDimensionsRef.current.height == 0) return  { minX: 0, maxX: 0, minY: 0, maxY: 0 };
  ...
  const calculateVisibleTiles = useCallback(() => {  
    if (containerDimensionsRef.current.width == 0 && containerDimensionsRef.current.height == 0) return [];
