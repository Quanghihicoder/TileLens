...
const userId = reduxUserId ?? JSON.parse(localStorage.getItem("user") || "{}").id;
...
const [isTilesOverflow, setIsTilesOverflow] = useState({ x: false, y: false });
...
const getNumberOfLevelsForImage = useCallback((width: number, height: number) => {
  const maxDimension = Math.max(width, height);
  return Math.ceil(1 + Math.log10(maxDimension));
}, []);

const levelMaxDimension = useMemo(() => {
  return Math.ceil(Math.max(imageWidth, imageHeight) / 2 ** (getNumberOfLevelsForImage(imageWidth, imageHeight) - 1 - zoom));
...
useEffect(() => {
  ...
  const fetchImage = async () => {
    try {
      const response = await axios.get(`${apiUrl}/image/${userId}/${imageId}`, {
      ...
      if (response.data?.image) {
        ...
        setImageName(image.imageOriginalName);
...
useEffect(() => {
  if (!widthOverflow && !heightOverflow) {
    ...
    if (isTilesOverflow.x && isTilesOverflow.y) {
  ...
  return (
    <>
      ...
      <div ref={windowContainerRef} className="h-full w-full max-w-[85vw] max-h-[90vh]">
        <div
          ref={tilesContainerRef}
          className={`h-full w-full overflow-hidden select-none no-scrollbar ${!isTilesOverflow.x && "flex justify-center"} ${!isTilesOverflow.y && "flex items-center"} `}
        >
          <div
