resource "aws_acm_certificate" "backend_cert" {
  ...
  tags = {
    Name = "${var.project_name}-backend-api-cert"
  }
}

locals {
  domain_validation_option = tolist(aws_acm_certificate.backend_cert.domain_validation_options)[0]
}
...
resource "aws_acm_certificate_validation" "backend_cert_validation" {
  ...
  validation_record_fqdns = [aws_route53_record.backend_cert_validation.fqdn]
}
